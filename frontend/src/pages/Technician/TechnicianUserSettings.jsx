...
const TechnicianUserSettings = ({ onClose }) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        console.log("Fetching technician profile data for user ID:", userId);

        // Use the userService to get the current user's profile
        const response = await API.get(`/users/${userId}`);
        console.log("Technician profile API response:", response.data);
        ...
        if (userData) {
          ...
          // Handle different field naming conventions in the API response
          setFormData({
          ...
          if (userData.profile_image) {
            // Check if the profile_image is already a full URL or just a filename
            ...
            // Some API responses might include a full URL in profile_image_url
            console.log("Setting profile image URL:", userData.profile_image_url);
            setImagePreview(userData.profile_image_url);
          }
        } else {
          console.error("Could not parse user data from response:", response.data);
        ...
        if (error.response && error.response.status === 401) {
          ...
          setErrors(prev => ({
            ...prev,
            apiError: "You don't have permission to access this profile."
          }));
        } else {
          setErrors(prev => ({
            ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("Updating technician profile for user ID:", userId);
      ...
      // Log the form data being sent (excluding the file for brevity)
      console.log("Submitting form data:", {
        ...
        hasPasswordChange: !!(formData.currentPassword && formData.newPassword),
        hasNewImage: !!(formData.profile_image && formData.profile_image instanceof File)
      });
      ...
      if (updatedUser) {
        console.log("Updating form with response data:", updatedUser);
        ...
        setFormData(prev => ({
          ...
          first_name: updatedUser.first_name || updatedUser.firstName || prev.first_name,
          last_name: updatedUser.last_name || updatedUser.lastName || prev.last_name,
        ...
        if (updatedUser.profile_image) {
          ...
          console.log("Updating profile image preview after save:", imageUrl);
          setImagePreview(imageUrl);
        } else if (updatedUser.profile_image_url) {
          console.log("Updating profile image URL after save:", updatedUser.profile_image_url);
          setImagePreview(updatedUser.profile_image_url);
        }
      } else {
        // If we can't parse the updated user data, fetch it again
        console.log("Could not parse updated user data from response, fetching fresh data");
        const refreshResponse = await API.get(`/users/${userId}`);

        if (refreshResponse.data && refreshResponse.data.data && refreshResponse.data.data.user) {
          const refreshedUser = refreshResponse.data.data.user;

          setFormData(prev => ({
            ...prev,
            username: refreshedUser.Username || refreshedUser.username || prev.username,
            email: refreshedUser.Email || refreshedUser.email || prev.email,
            phone: refreshedUser.Phone || refreshedUser.phone || prev.phone,
            first_name: refreshedUser.first_name || refreshedUser.firstName || prev.first_name,
            last_name: refreshedUser.last_name || refreshedUser.lastName || prev.last_name,
            profile_image: refreshedUser.profile_image || prev.profile_image
          }));

          // Update image preview if needed
          if (refreshedUser.profile_image) {
            const imageUrl = refreshedUser.profile_image.startsWith('http')
              ? refreshedUser.profile_image
              : `http://localhost:5000/uploads/${refreshedUser.profile_image}`;

            setImagePreview(imageUrl);
          } else if (refreshedUser.profile_image_url) {
            setImagePreview(refreshedUser.profile_image_url);
          }
        }
      }
    } catch (error) {
      ...
      if (error.response?.data?.message) {
        ...
        setErrors({
          apiError: "You don't have permission to update this profile."
        });
      } else if (error.response?.status === 413) {
        setErrors({
          profile_image: "The image file is too large. Please select a smaller image (max 2MB)."
        });
      } else {
        setErrors({
  ...
  const handleCancel = () => {
    ...
    const fetchUserData = async () => {
      try {
        console.log("Resetting technician profile data for user ID:", userId);

        const response = await API.get(`/users/${userId}`);
        console.log("Reset profile API response:", response.data);
        ...
        if (userData) {
          ...
          if (userData.profile_image) {
            ...
            console.log("Resetting profile image preview:", imageUrl);
            setImagePreview(imageUrl);
          } else if (userData.profile_image_url) {
            // Some API responses might include a full URL in profile_image_url
            console.log("Resetting profile image URL:", userData.profile_image_url);
            setImagePreview(userData.profile_image_url);
          } else {
            setImagePreview(null);
          }
        } else {
          console.error("Could not parse user data from reset response:", response.data);
          throw new Error("Could not parse user data from response");
        }
      } catch (error) {
        console.error("Error resetting technician profile data:", error);
        console.error("Error details:", error.response?.data || error.message);

        // If error occurs, just reset the password fields and show an error
        setFormData(prev => ({
          ...prev,
          currentPassword: "",
          newPassword: "",
        ...
        setErrors(prev => ({
          ...prev,
          apiError: "Failed to reset form data. Some fields may not be updated correctly."
        }));
      }
    };
              ...
              <div className="relative">
                {imagePreview ? (
                  ...
                  <div
                    ...
                    onClick={triggerFileInput}
                  >
                    {formData.username ? formData.username.substring(0, 2).toUpperCase() : "TU"}
                    {isEditing && (
...
export default TechnicianUserSettings;